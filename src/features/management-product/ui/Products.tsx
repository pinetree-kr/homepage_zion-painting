...
export default function Products({
  ...
  const addProduct = () => {
    router.push('/admin/sections/products/new');
  };

  const editProduct = (product: Product & { category?: ProductCategory | null }) => {
    router.push(`/admin/sections/products/${product.id}`);
  };
      ...
      accessor: (row) => {
        return (
          <div className="flex items-center gap-2">
            ...
            <Link href={`/admin/sections/products/${row.id}`} className="text-blue-500 hover:text-blue-700">
              {row.title}
            </Link>
          </div>
        );
