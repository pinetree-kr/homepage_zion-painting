...
import { Plus, Trash2, Edit, Calendar, Tag, Image as ImageIcon } from 'lucide-react';
...
export default function BusinessAchievements({
  ...
  const addAchievement = () => {
    router.push('/admin/site-settings/business/achievements/new');
  };

  const editAchievement = (achievement: Achievement & { category?: BusinessCategory | null }) => {
    router.push(`/admin/site-settings/business/achievements/${achievement.id}`);
  };
      ...
      accessor: (row) => {
        return (
          <div className="flex items-center gap-2">
            ...
            <Link href={`/admin/site-settings/business/achievements/${row.id}`} className="text-blue-500 hover:text-blue-700">
              {row.title}
            </Link>
          </div>
        );
